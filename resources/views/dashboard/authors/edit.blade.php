...
<div class="max-w-2xl mx-auto px-4 py-10">
    <h1 class="font-display text-2xl font-semibold text-ink mb-6">Edit author</h1>

    <form action="{{ route('dashboard.authors.update', $author) }}" method="POST" class="bg-paper border border-line p-6 space-y-5">
        @csrf
        @method('PUT')
        ...
        <div>
            ...
            <textarea name="bio" rows="4" class="w-full border border-line rounded-sm px-3 py-2 bg-white">{{ old('bio', $author->bio) }}</textarea>
        ...
        <div class="flex justify-end gap-3 pt-2">
            ...
            <button type="submit" class="bg-forest hover:bg-forest-dark text-paper px-5 py-2 rounded-sm transition">Update author</button>
        </div>
    </form>
</div>
@endsection
